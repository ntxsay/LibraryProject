...
namespace LibraryProjectUWP.Code.Services.Tasks
{
    public class GetBookPretsTask
    {
        ...
        public delegate void AfterTaskCompletedEventHandler(GetBookPretsTask sender, RunWorkerCompletedEventArgs e);
        public event AfterTaskCompletedEventHandler AfterTaskCompletedRequested;

        public GetBookPretsTask()
        {
            UseBusyLoader = false;
            CloseBusyLoaderAfterFinish = false;
        }

        public GetBookPretsTask(MainPage mainPage)
        {
            MainPage = mainPage;
        }
                ...
                if (WorkerBackground != null)
                {
                    if (!WorkerBackground.IsBusy)
                    {
                        ...
                        if(UseBusyLoader)
                        {
                            ...
                            {
                                ProgessText = $"Recherche en cours d'exemplaires du livre « {viewModel.MainTitle} » empruntés. Veuillez patienter quelques instants.",
        ...
        private void WorkerBackground_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                {
                    using (Task<IList<LivrePretVM>> task = DbServices.BookPret.GetBookPretVMAsync(viewModel.Id, BookPretFrom.Book, cancellationTokenSource.Token))
                    {
                        task.Wait();
