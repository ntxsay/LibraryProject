...
{
    public class GetBookExemplariesTask
    {
        ...
        public delegate void AfterTaskCompletedEventHandler(GetBookExemplariesTask sender, RunWorkerCompletedEventArgs e);
        public event AfterTaskCompletedEventHandler AfterTaskCompletedRequested;

        public GetBookExemplariesTask()
        {
            UseBusyLoader = false;
            CloseBusyLoaderAfterFinish = false;
        }

        public GetBookExemplariesTask(MainPage mainPage)
        {
            MainPage = mainPage;
        }
                ...
                if (WorkerBackground != null)
                {
                    if (!WorkerBackground.IsBusy)
                    {
                        ...
                        if(UseBusyLoader)
                        {
                            ...
                            {
                                ProgessText = $"Recherche en cours d'exemplaires du livre « {viewModel.MainTitle} ». Veuillez patienter quelques instants.",
        ...
        private void WorkerBackground_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                {
                    using (Task<IList<LivreExemplaryVM>> task = DbServices.Book.GetBookExemplaryVMAsync(viewModel.Id, cancellationTokenSource.Token))
                    {
                        task.Wait();
        ...
        private void WorkerBackground_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            try
            {
                ...
                if (e.Error != null)
                {
                    message = $"Une erreur s'est produite lors de la recherche d'exemplaire(s)";
                }
                else if (e.Cancelled)
                {
                    message = $"La recherche d'exemplaire(s) a été annulée par l'utilisateur.";
                }
                else
                {
                    var result = e.Result as Tuple<LivreVM, WorkerState<LivreExemplaryVM, LivreExemplaryVM>>;
                    message = $"Le livre {result?.Item1?.MainTitle ?? "??"} contient {result.Item2.ResultList?.Count() ?? 0} exemplaire(s)";
                }
