...
{
    public class GetCollectionsTask
    {
        ...
        public delegate void AfterTaskCompletedEventHandler(GetCollectionsTask sender, RunWorkerCompletedEventArgs e);
        ...
        public GetCollectionsTask(MainPage mainPage)
        {
            MainPage = mainPage;
        }
                ...
                if (WorkerBackground != null)
                {
                    if (!WorkerBackground.IsBusy)
                    {
                        ...
                        if(UseBusyLoader)
                        {
                            ...
                            {
                                ProgessText = $"Recherche en cours des collections de la bibliothèque « {viewModel.Name} ». Veuillez patienter quelques instants.",
        ...
        private void WorkerBackground_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                {
                    using (Task<IList<CollectionVM>> task = DbServices.Collection.MultipleVmInLibraryAsync(viewModel.Id, CollectionTypeEnum.Collection, cancellationTokenSource.Token))
                    {
                        task.Wait();
                        ...
                        long countNotInCollectionBooks = 0;
                        using (Task<long> task2 = DbServices.Collection.CountUnCategorizedBooks(viewModel.Id))
                        {
                            task2.Wait();
                            countNotInCollectionBooks = task2.Result;
                        }

                        var result = task.Result;
                        var state = new WorkerState<CollectionVM, CollectionVM>()
                        {
                            ResultList = result?.OrderBy(o => o.Name),
                        };

                        e.Result = new Tuple<BibliothequeVM, WorkerState<CollectionVM, CollectionVM>, long>(viewModel, state, countNotInCollectionBooks);
                    }
                }
            }
            catch (Exception ex)
        ...
        private void WorkerBackground_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            try
            {
                ...
                {
                    var result = e.Result as Tuple<BibliothequeVM, WorkerState<CollectionVM, CollectionVM>, long>;
                    message = $"La bibliothèque {result?.Item1?.Name ?? "??"} contient {result.Item2.ResultList?.Count() ?? 0} collection(s).";
                }
